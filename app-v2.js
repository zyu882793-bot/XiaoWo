/* TB社区浏览器包：Supabase 2.115.0 + 应用代码。请运行 build-bundle.ps1 重新生成。 */
...
(function () {
  ...
  function errorText(error) {
    ...
    if (/email_address_invalid|email address.*invalid/i.test(message)) return '邮箱不可用';
    if (/signup_disabled|signups? not allowed/i.test(message)) return '注册已关闭';
    ...
    if (/insufficient balance/i.test(message)) return '余额不足';
    if (/friend.*required|only.*friend|not friends|limited to friends/i.test(message)) return '仅可向好友转账';
    if (/recipient.*not found|recipient is unavailable|invalid recipient|user not found/i.test(message)) return '用户不存在';
    if (/transfer.*yourself|recipient.*yourself/i.test(message)) return '不能转给自己';
    if (/invalid amount|amount.*positive|amount.*range/i.test(message)) return '金额无效';
    if (/deletion.*not ready|waiting period|deletion wait period/i.test(message)) return '仍在等待期';
    if (/under maintenance/i.test(message)) return '页面维护中';
    if (/pending withdrawals/i.test(message)) return '请先处理提现';
    if (/last active admin|assign another admin/i.test(message)) return '请先指定其他管理员';
    if (/reason.*(?:contain|characters|long)/i.test(message)) return '原因格式错误';
    if (/destination.*(?:required|empty)/i.test(message)) return '请填写收款账户';
    if (/destination.*long/i.test(message)) return '收款信息过长';
    if (/note.*long/i.test(message)) return '备注过长';
    if (/balance cannot become negative/i.test(message)) return '余额不足';
    if (/withdrawal.*(?:not found|already reviewed)/i.test(message)) return /reviewed/i.test(message) ? '申请已处理' : '申请不存在';
    if (/pending deletion request not found/i.test(message)) return '申请不存在';
    if (/invalid theme preset/i.test(message)) return '主题无效';
    if (/client id is required/i.test(message)) return '请求无效';
    if (/client id was already used/i.test(message)) return '请求冲突';
    if (/you cannot delete your own|cannot delete yourself/i.test(message)) return '不能注销自己';
    if (/background must be|overlay is out of range/i.test(message)) return '背景设置无效';
    if (/site title must/i.test(message)) return '标题无效';
    if (/admin access required/i.test(message)) return '需要管理员权限';
    return '请求失败';
  }
  ...
  function setBusy(button, busy, busyText) {
    ...
    if (busy) {
      ...
      button.dataset.richLabel = button.childElementCount ? 'true' : 'false';
      if (button.dataset.richLabel === 'true') {
        button.setAttribute('aria-label', busyText || '处理中');
      ...
      if (button.dataset.richLabel !== 'true' && button.dataset.label) button.textContent = button.dataset.label;
      button.removeAttribute('aria-label');
      button.removeAttribute('aria-busy');
      delete button.dataset.label;
      delete button.dataset.richLabel;
    }
  }
  ...
  function requestClientId(key, fingerprint) {
    var signature = JSON.stringify(fingerprint);
    var current = state.requestIntents[key];
    if (!current || current.signature !== signature) {
      current = { signature: signature, clientId: uuid() };
      state.requestIntents[key] = current;
    }
    return current.clientId;
  }

  function clearRequestClientId(key) {
    delete state.requestIntents[key];
  }
  ...
  async function resendSignupCode() {
    ...
    try {
      var resend = await db.auth.resend({ type: 'signup', email: state.authOtp.email });
  ...
  function applySettings(settings) {
    ...
    var themes = ['custom', 'spring', 'festival', 'summer', 'autumn', 'school', 'winter'];
    var theme = themes.includes(settings.theme_preset) ? settings.theme_preset : 'custom';
    var pageMaintenance = settings.page_maintenance || {};
    if (typeof pageMaintenance === 'string') {
      try { pageMaintenance = JSON.parse(pageMaintenance); } catch (ignore) { pageMaintenance = {}; }
    }
    document.documentElement.dataset.theme = theme;
    if (document.body) document.body.dataset.siteTheme = theme;
    if (theme === 'custom') {
      ...
      var imageOpacity = background ? Math.max(.04, Math.min(.4, .4 * (1 - (Number.isFinite(overlay) ? overlay : .35)))) : 0;
      ...
      document.documentElement.style.removeProperty('--site-image-opacity');
    }
    state.settings = Object.assign({}, settings, {
      site_title: title,
      theme_preset: theme,
      page_maintenance: pageMaintenance && typeof pageMaintenance === 'object' ? pageMaintenance : {}
    });
  }
  ...
  function setAccount(profile) {
    ...
    ['admin-users-nav', 'admin-wallet-nav', 'admin-settings-nav', 'admin-logs-nav'].forEach(function (id) {
      var node = byId(id);
      if (node) node.classList.toggle('hidden', !admin);
    });
  }
  ...
  function clearPrivateState() {
    window.clearInterval(state.deletionTimer);
    state.deletionTimer = 0;
    state.profile = null;
    ...
    state.friendsToken += 1;
    state.messageToken += 1;
    state.messageBusyPeer = '';
    state.deletionToken += 1;
    state.requestIntents = {};
    state.wallet = null;
    state.walletTransactions = [];
    state.walletToken += 1;
    state.walletAdminUser = null;
    state.walletAdminUsers = [];
    state.adminWalletToken += 1;
    ...
    [
      'post-form', 'profile-form', 'chat-form', 'friend-form', 'report-form', 'comment-form',
      'transfer-form', 'recharge-form', 'withdraw-form', 'admin-balance-form', 'admin-wallet-search-form'
    ].forEach(function (id) {
      var form = byId(id);
      if (form) form.reset();
    });
    if (byId('auth-password')) byId('auth-password').value = '';
    if (byId('auth-code')) byId('auth-code').value = '';
    [
      ['feed', '暂无动态'], ['profile-posts', '暂无动态'], ['profile-favorites', '暂无收藏'],
      ['friends-list', '暂无好友'], ['notification-list', '暂无提醒'], ['chat-list', '选择好友'],
      ['wallet-bill-list', '暂无账单'], ['admin-wallet-user-list', '查询用户'],
      ['admin-account-bill-list', '选择账户'], ['admin-withdrawal-list', '暂无申请'],
      ['review-list', '暂无待审'], ['report-list', '暂无举报'], ['user-list', '暂无用户'], ['log-list', '暂无日志']
    ].forEach(function (item) {
      if (byId(item[0])) empty(byId(item[0]), item[1]);
    });
    if (byId('wallet-balance')) byId('wallet-balance').textContent = '¥0.00';
    if (byId('wallet-account-id')) byId('wallet-account-id').textContent = '—';
    if (byId('account-name')) byId('account-name').textContent = '账户';
    if (byId('account-role')) byId('account-role').textContent = '';
    if (byId('chat-peer-name')) byId('chat-peer-name').textContent = '选择好友';
    if (byId('profile-name')) byId('profile-name').textContent = '用户';
    if (byId('profile-username')) byId('profile-username').textContent = '';
    if (byId('profile-bio')) byId('profile-bio').textContent = '';
    if (byId('profile-following')) byId('profile-following').textContent = '0';
    if (byId('profile-followers')) byId('profile-followers').textContent = '0';
    document.querySelectorAll('[data-avatar]').forEach(function (node) { setAvatar(node, { username: 'TB' }); });
    resetAdminWalletUser();
  ...
  function startSettingsRefresh() {
    window.clearInterval(state.settingsTimer);
    state.settingsToken += 1;
    state.settingsTimer = window.setInterval(async function () {
      if (!state.session) return;
      var userId = state.session.user.id;
      var requestToken = ++state.settingsToken;
      try {
        var active = state.activePage || 'home';
        var beforePage = !!(state.settings && state.settings.page_maintenance && state.settings.page_maintenance[active]);
        var settings = first(await rpc('get_public_site_settings_v2')) || {};
        if (requestToken !== state.settingsToken || !state.session || state.session.user.id !== userId) return;
        applySettings(settings);
        var globalBlocked = !!settings.maintenance_mode && state.profile && state.profile.role !== 'admin';
        if (globalBlocked) {
          hide(byId('view-app'));
          show(byId('view-maintenance'));
          return;
        }
        var wasGlobalBlocked = !byId('view-maintenance').classList.contains('hidden');
        hide(byId('view-maintenance'));
        show(byId('view-app'));
        var afterPage = !!(state.settings.page_maintenance && state.settings.page_maintenance[active]);
        if (wasGlobalBlocked || beforePage !== afterPage) await switchPage(active);
      } catch (ignore) {}
    }, 15000);
  }
  ...
  async function route() {
    ...
    try {
      ...
      hide(byId('view-auth'));
      if (settings.maintenance_mode && state.profile.role !== 'admin') {
        show(byId('view-maintenance'));
        return;
      }
      show(byId('view-app'));
  ...
  async function toggleFavorite() {
    ...
    try {
      ...
      button.dataset.label = active ? '已收藏' : '收藏';
  ...
  async function toggleFollow() {
    ...
    try {
      ...
      button.dataset.label = active ? '已关注' : '关注';
  ...
  async function resetPassword() {
    ...
    var result = await db.auth.resetPasswordForEmail(email, { redirectTo: PUBLIC_URL });
  ...
  function amountFrom(id) {
    ...
    if (!/^(?:0|[1-9]\d{0,11})(?:\.\d{1,2})?$/.test(raw) || Number(raw) <= 0) throw new Error('Invalid amount');
    return Number(raw);
  }

  function billLabel(type) {
    return ({
      transfer_out: '转账支出', transfer_in: '转账收入',
      admin_credit: '管理员入账', admin_debit: '管理员扣款', admin_adjustment: '余额调整',
      withdrawal_hold: '提现申请', withdrawal_request: '提现申请',
      withdrawal_refund: '提现退回', withdrawal_paid: '提现完成'
    })[type] || '账户变动';
  }

  function billIsCredit(row) {
    var amount = Number(row.amount);
    if (Number.isFinite(amount) && amount !== 0) return amount > 0;
    return /(?:_in|credit|refund|deposit)$/.test(row.transaction_type || '');
  }

  function renderBills(container, rows, includeUser) {
    if (!container) return;
    if (!rows || !rows.length) return empty(container, '暂无账单');
    container.replaceChildren();
    rows.forEach(function (row) {
      var title = billLabel(row.transaction_type);
      if (includeUser && row.username) title += ' · @' + row.username;
      if (row.counterparty_username) title += ' · @' + row.counterparty_username;
      var item = adminCard(title, row.description || '');
      var amount = document.createElement('strong');
      var credit = billIsCredit(row);
      amount.className = 'bill-amount ' + (credit ? 'credit' : 'debit');
      amount.textContent = (credit ? '+' : '-') + money(Math.abs(Number(row.amount || 0)));
      var time = document.createElement('small');
      time.textContent = formatTime(row.created_at) + ' · 余额 ' + money(row.balance_after);
      item.head.append(amount, time);
      container.appendChild(item.card);
    });
  }

  async function loadWallet() {
    var token = ++state.walletToken;
    var balanceNode = byId('wallet-balance');
    if (balanceNode) balanceNode.textContent = '加载中';
    try {
      var values = await Promise.all([
        rpc('get_my_wallet_v2'),
        rpc('list_my_wallet_transactions_v2', { p_limit: 100, p_before: null })
      ]);
      if (token !== state.walletToken) return;
      state.wallet = first(values[0]) || { balance: 0 };
      state.walletTransactions = values[1] || [];
      if (balanceNode) balanceNode.textContent = money(state.wallet.balance);
      if (byId('wallet-account-id')) byId('wallet-account-id').textContent = '@' + state.profile.username;
      if (byId('transfer-balance')) byId('transfer-balance').textContent = money(state.wallet.balance);
      if (byId('withdraw-balance')) byId('withdraw-balance').textContent = money(state.wallet.balance);
      renderBills(byId('wallet-bill-list'), state.walletTransactions, false);
      await loadDeletionRequest();
    } catch (error) {
      if (token !== state.walletToken) return;
      if (balanceNode) balanceNode.textContent = '--';
      empty(byId('wallet-bill-list'), errorText(error));
      toast(errorText(error));
    }
  }
  ...
  async function openTransferDialog() {
    ...
    await loadFriends();
    byId('transfer-recipient').value = '';
    byId('transfer-amount').value = '';
    byId('transfer-note').value = '';
    byId('transfer-balance').textContent = money(state.wallet && state.wallet.balance);
    openDialog('transfer-dialog');
    byId('transfer-recipient').focus();
  }

  async function submitTransfer(event) {
    event.preventDefault();
    var query = byId('transfer-recipient').value.trim().replace(/^@/, '');
    var friend = state.friends.find(function (item) {
      return item.user_id === query || String(item.id || '') === query || String(item.username || '').toLowerCase() === query.toLowerCase();
    });
    if (!friend) return toast('请选择好友');
    var amount;
    try { amount = amountFrom('transfer-amount'); } catch (error) { return toast(errorText(error)); }
    var note = byId('transfer-note').value.trim() || null;
    var clientId = requestClientId('transfer', [friend.user_id, amount.toFixed(2), note || '']);
    var button = byId('transfer-submit');
    setBusy(button, true, '转账中');
    try {
      await rpc('transfer_balance_v2', {
        p_recipient_id: friend.user_id,
        p_amount: amount,
        p_client_id: clientId,
        p_note: note
      });
      clearRequestClientId('transfer');
      closeDialog('transfer-dialog');
      toast('转账成功');
      await loadWallet();
    } catch (error) { toast(errorText(error)); }
    finally { setBusy(button, false); }
  ...
  function openRechargeDialog() {
    byId('recharge-amount').value = '10';
    document.querySelectorAll('[data-recharge-amount]').forEach(function (button) {
      button.classList.toggle('active', button.dataset.rechargeAmount === '10');
    });
    openDialog('recharge-dialog');
  }

  function submitRecharge(event) {
    event.preventDefault();
    try { amountFrom('recharge-amount'); } catch (error) { return toast(errorText(error)); }
    closeDialog('recharge-dialog');
    toast('充值暂未开放');
  }
  ...
  async function openWithdrawDialog() {
    ...
    byId('withdraw-account').value = '';
    byId('withdraw-balance').textContent = money(state.wallet && state.wallet.balance);
    openDialog('withdraw-dialog');
  }

  async function submitWithdrawal(event) {
    event.preventDefault();
    var amount;
    try { amount = amountFrom('withdraw-amount'); } catch (error) { return toast(errorText(error)); }
    var destination = byId('withdraw-account').value.trim() || null;
    var clientId = requestClientId('withdrawal', [amount.toFixed(2), destination || '']);
    var button = byId('withdraw-submit');
    setBusy(button, true, '提交中');
    try {
      await rpc('request_withdrawal_v2', {
        p_amount: amount,
        p_destination: destination,
        p_client_id: clientId
      });
      clearRequestClientId('withdrawal');
  ...
  function rememberDeletionRequest(value) {
    var request = first(value);
    if (request && Number.isFinite(Number(request.seconds_remaining))) {
      request.client_ready_at = Date.now() + Math.max(0, Number(request.seconds_remaining)) * 1000;
    }
    return request;
  }

  function deletionSeconds(request) {
    if (!request) return 0;
    if (request.client_ready_at) return Math.max(0, Math.ceil((request.client_ready_at - Date.now()) / 1000));
    if (request.execute_after) return Math.max(0, Math.ceil((new Date(request.execute_after).getTime() - Date.now()) / 1000));
    return Math.max(0, Number(request.seconds_remaining || 0));
  }

  function updateDeletionCountdown() {
    var seconds = deletionSeconds(state.deletionRequest);
    var countdown = byId('account-delete-countdown');
    var confirm = byId('account-delete-confirm');
    if (countdown) countdown.textContent = seconds ? Math.floor(seconds / 60) + '分' + String(seconds % 60).padStart(2, '0') + '秒' : '可以注销';
    if (confirm) confirm.disabled = seconds > 0;
    if (!seconds && state.deletionTimer) {
      window.clearInterval(state.deletionTimer);
      state.deletionTimer = 0;
    }
  }
  ...
  function renderDeletionRequest() {
    window.clearInterval(state.deletionTimer);
    state.deletionTimer = 0;
    var pending = Boolean(state.deletionRequest && state.deletionRequest.status === 'pending');
    byId('account-delete-request').classList.toggle('hidden', pending);
    byId('account-delete-pending').classList.toggle('hidden', !pending);
    byId('account-delete-status').textContent = pending ? '注销已申请' : '注销后无法恢复';
    if (!pending) return;
    updateDeletionCountdown();
    if (deletionSeconds(state.deletionRequest) > 0) state.deletionTimer = window.setInterval(updateDeletionCountdown, 1000);
  }

  async function loadDeletionRequest() {
    var userId = sessionUserId();
    if (!userId) return;
    var token = ++state.deletionToken;
    try {
      var request = rememberDeletionRequest(await rpc('get_my_account_deletion_v2'));
      if (token !== state.deletionToken || sessionUserId() !== userId) return;
      state.deletionRequest = request;
      renderDeletionRequest();
    } catch (error) {
      if (token !== state.deletionToken || sessionUserId() !== userId) return;
      if (byId('account-delete-status')) byId('account-delete-status').textContent = errorText(error);
    }
  }

  function requestAccountDeletion() {
    confirmAction('申请注销', '10 分钟后删除账户。', async function () {
      state.deletionRequest = rememberDeletionRequest(await rpc('request_account_deletion_v2'));
      renderDeletionRequest();
      toast('注销已申请');
    });
  }
  ...
  async function cancelAccountDeletion() {
    try {
      await rpc('cancel_account_deletion_v2');
      state.deletionRequest = null;
      renderDeletionRequest();
      toast('已取消');
    } catch (error) { toast(errorText(error)); }
  ...
  function finalizeAccountDeletion() {
    confirmAction('注销账户', '账户和余额将永久删除。', async function () {
      await rpc('finalize_my_account_deletion_v2');
      window.clearInterval(state.deletionTimer);
  ...
  async function loadNotifications(silent) {
    var userId = sessionUserId();
    if (!userId || state.notificationBusy) return;
    var token = ++state.notificationToken;
    ...
    try {
      var result = await db.from('sys_notifications')
        ...
        .limit(80);
      if (result.error) throw result.error;
      if (token !== state.notificationToken || sessionUserId() !== userId) return;
      ...
      if (!silent && token === state.notificationToken && sessionUserId() === userId) toast(errorText(error));
    } finally {
      if (token === state.notificationToken) state.notificationBusy = false;
    }
  }
  ...
  async function startNotifications() {
    ...
    if (state.notificationChannel) await db.removeChannel(state.notificationChannel);
    if (sessionUserId() !== userId) return;
    state.notificationChannel = db.channel('tb-notifications-' + userId)
      .on('postgres_changes', {
  ...
  async function loadFriends() {
    var userId = sessionUserId();
    if (!userId) return;
    var token = ++state.friendsToken;
    try {
      var friends = (await rpc('list_friends_v2')) || [];
      if (token !== state.friendsToken || sessionUserId() !== userId) return;
      ...
      if (token === state.friendsToken && sessionUserId() === userId) toast(errorText(error));
    }
  }
  ...
  function renderMessage(message) {
    ...
    var image = '';
    if (/^data:image\/(?:png|jpe?g|webp|gif);base64,/i.test(content) ||
        /^https:\/\/[^\s?#]+\.(?:png|jpe?g|webp|gif)(?:[?#].*)?$/i.test(content)) image = validUrl(content);
    if (image) bubble.appendChild(imageNode(image, '聊天图片'));
  ...
  async function loadConversation(silent) {
    var userId = sessionUserId();
    var peerId = state.activePeer && state.activePeer.user_id;
    if (!userId || !peerId) return;
    if (state.messageBusyPeer === peerId) return;
    state.messageBusyPeer = peerId;
    ...
    try {
      ...
      if (token !== state.messageToken || sessionUserId() !== userId ||
          !state.activePeer || state.activePeer.user_id !== peerId) return;
      ...
      if (!silent && token === state.messageToken && sessionUserId() === userId) toast(errorText(error));
    } finally {
      if (token === state.messageToken && state.messageBusyPeer === peerId) state.messageBusyPeer = '';
    }
  }
  ...
  async function startMessageStream() {
    ...
    if (state.messageChannel) await db.removeChannel(state.messageChannel);
    if (sessionUserId() !== userId) return;
    state.messageChannel = db.channel('tb-messages-' + userId)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'sys_messages' }, function (payload) {
        var row = payload.new || {};
        if (!state.activePeer) return;
        if ((row.sender_id === userId && row.receiver_id === state.activePeer.user_id) ||
            (row.receiver_id === userId && row.sender_id === state.activePeer.user_id)) loadConversation(true);
      })
      .subscribe();
    window.clearInterval(state.messageTimer);
  ...
  async function openChat(friend) {
    ...
    if (!state.activePeer || state.activePeer.user_id !== friend.user_id) return;
  ...
  async function sendMessage(event) {
    ...
    if (!content) return;
    var userId = sessionUserId();
    var peerId = state.activePeer.user_id;
    var intentKey = 'message:' + peerId;
    var clientId = requestClientId(intentKey, [peerId, content]);
    var button = byId('chat-send');
    setBusy(button, true, '发送中');
    input.value = '';
    try {
      await rpc('send_message_v2', { p_receiver_id: peerId, p_content: content, p_client_id: clientId });
      clearRequestClientId(intentKey);
      if (sessionUserId() === userId && state.activePeer && state.activePeer.user_id === peerId) {
        state.messageSignature = '';
        await loadConversation(true);
      }
    } catch (error) {
      if (sessionUserId() === userId && state.activePeer && state.activePeer.user_id === peerId && !input.value) input.value = content;
      if (sessionUserId() === userId) toast(errorText(error));
    } finally {
      setBusy(button, false);
      if (sessionUserId() === userId && state.activePeer && state.activePeer.user_id === peerId) input.focus();
    }
  }
  ...
  async function sendChatImage() {
    ...
    var userId = sessionUserId();
    var peerId = state.activePeer.user_id;
    var intentKey = 'message-image:' + peerId;
    var clientId = requestClientId(intentKey, [peerId, file.name, file.size, file.lastModified]);
    try {
      var content = await compressImage(file, 1280, .8);
      await rpc('send_message_v2', { p_receiver_id: peerId, p_content: content, p_client_id: clientId });
      clearRequestClientId(intentKey);
      byId('chat-file').value = '';
      if (state.activePeer && state.activePeer.user_id === peerId) {
        state.messageSignature = '';
        await loadConversation(true);
      }
    } catch (error) { if (sessionUserId() === userId) toast(errorText(error)); }
  }
  ...
  async function loadUsers() {
    ...
    try {
      ...
      rows.forEach(function (row) {
        ...
        role.value = row.role;
        actions.append(role, actionButton('账户', false, function () {
          openAdminWalletForUser(row);
        }), actionButton(row.is_banned ? '解封' : '封禁', false, function () {
  ...
  function adminWalletUserButton(row) {
    var button = document.createElement('button');
    button.type = 'button';
    button.className = 'admin-wallet-user';
    button.dataset.userId = row.user_id;
    button.setAttribute('aria-pressed', String(Boolean(state.walletAdminUser && state.walletAdminUser.user_id === row.user_id)));
    var name = document.createElement('strong');
    name.textContent = row.nickname || row.username || '用户';
    var meta = document.createElement('small');
    meta.textContent = row.email || ('@' + (row.username || '用户'));
    button.append(name, meta);
    button.addEventListener('click', function () { selectAdminWalletUser(row); });
    return button;
  }

  async function loadAdminWalletUsers(search) {
    var container = byId('admin-wallet-user-list');
    empty(container, '加载中');
    try {
      var rows = (await rpc('admin_list_users_v2', { p_search: search || null })) || [];
      state.walletAdminUsers = rows;
      if (!rows.length) {
        state.walletAdminUser = null;
        resetAdminWalletUser();
        return empty(container, '未找到用户');
      }
      container.replaceChildren();
      rows.forEach(function (row) { container.appendChild(adminWalletUserButton(row)); });
      var selected = rows.find(function (row) {
        return state.walletAdminUser && row.user_id === state.walletAdminUser.user_id;
      }) || rows[0];
      await selectAdminWalletUser(selected);
    } catch (error) {
      empty(container, errorText(error));
      throw error;
    }
  }

  function resetAdminWalletUser() {
    state.adminWalletToken += 1;
    byId('admin-account-name').textContent = '未选择';
    byId('admin-account-uid').textContent = '—';
    byId('admin-account-email').textContent = '—';
    byId('admin-account-balance').textContent = '¥0.00';
    byId('admin-account-delete').disabled = true;
    byId('admin-balance-submit').disabled = true;
    empty(byId('admin-account-bill-list'), '选择账户');
  }

  async function selectAdminWalletUser(row) {
    if (!row) return resetAdminWalletUser();
    var token = ++state.adminWalletToken;
    state.walletAdminUser = row;
    byId('admin-balance-form').reset();
    document.querySelectorAll('.admin-wallet-user').forEach(function (button) {
      var active = button.dataset.userId === row.user_id;
      button.classList.toggle('active', active);
      button.setAttribute('aria-pressed', String(active));
    });
    byId('admin-account-name').textContent = row.nickname || row.username || '用户';
    byId('admin-account-uid').textContent = 'UID ' + (row.id || row.user_id);
    byId('admin-account-email').textContent = row.email || '未提供邮箱';
    byId('admin-account-balance').textContent = '加载中';
    byId('admin-account-delete').disabled = false;
    byId('admin-balance-submit').disabled = false;
    empty(byId('admin-account-bill-list'), '加载中');
    try {
      var values = await Promise.all([
        rpc('admin_get_user_wallet_v2', { p_user_id: row.user_id }),
        rpc('admin_list_wallet_transactions_v2', { p_user_id: row.user_id, p_limit: 100, p_before: null })
      ]);
      var wallet = first(values[0]) || { balance: 0 };
      if (token !== state.adminWalletToken || !state.walletAdminUser || state.walletAdminUser.user_id !== row.user_id) return;
      byId('admin-account-balance').textContent = money(wallet.balance);
      renderBills(byId('admin-account-bill-list'), values[1] || [], false);
    } catch (error) {
      if (token !== state.adminWalletToken) return;
      byId('admin-account-balance').textContent = '--';
      empty(byId('admin-account-bill-list'), errorText(error));
      toast(errorText(error));
    }
  }

  async function loadAdminWithdrawals() {
    var container = byId('admin-withdrawal-list');
    empty(container, '加载中');
    try {
      var rows = (await rpc('admin_list_withdrawals_v2', { p_status: 'pending', p_limit: 100 })) || [];
      if (!rows.length) return empty(container, '暂无申请');
      container.replaceChildren();
      rows.forEach(function (row) {
        var name = row.nickname || row.username || row.requester_username || '用户';
        var item = adminCard(name + ' · ' + money(row.amount), row.destination || '未填写收款账户');
        var time = document.createElement('small');
        ...
        if ((row.status || 'pending') === 'pending') {
          var actions = document.createElement('div');
          actions.className = 'admin-actions';
          var withdrawalId = row.id || row.withdrawal_id;
          actions.append(
            actionButton('通过', true, function () { reviewWithdrawal(withdrawalId, 'approve'); }),
            actionButton('拒绝', false, function () { reviewWithdrawal(withdrawalId, 'reject'); })
          );
          item.card.appendChild(actions);
        }
        container.appendChild(item.card);
  ...
  async function loadAdminWalletPage(search) {
    await Promise.all([loadAdminWalletUsers(search), loadAdminWithdrawals()]);
  }

  async function submitAdminWalletSearch(event) {
    event.preventDefault();
    var button = byId('admin-wallet-search');
    setBusy(button, true, '查询中');
    try { await loadAdminWalletUsers(byId('admin-wallet-query').value.trim()); }
    catch (error) { toast(errorText(error)); }
    finally { setBusy(button, false); }
  }

  async function adjustAdminBalance(event) {
    event.preventDefault();
    if (!state.walletAdminUser) return toast('请选择账户');
    var raw = byId('admin-balance-amount').value.trim();
    if (!/^-?(?:0|[1-9]\d{0,11})(?:\.\d{1,2})?$/.test(raw) || !Number(raw)) return toast('金额无效');
    var reason = byId('admin-balance-reason').value.trim();
    if (!reason) return toast('请填写原因');
    var amount = Number(raw);
    var clientId = requestClientId('admin-balance', [state.walletAdminUser.user_id, amount.toFixed(2), reason]);
    var button = byId('admin-balance-submit');
    setBusy(button, true, '保存中');
    try {
      await rpc('admin_adjust_balance_v2', {
        p_user_id: state.walletAdminUser.user_id,
        p_amount: amount,
        p_reason: reason,
        p_client_id: clientId
      });
      clearRequestClientId('admin-balance');
      byId('admin-balance-form').reset();
      toast('余额已更新');
      await selectAdminWalletUser(state.walletAdminUser);
    } catch (error) { toast(errorText(error)); }
    finally { setBusy(button, false); }
  }

  async function reviewWithdrawal(id, decision) {
    if (!id) return toast('申请不可用');
    try {
      await rpc('admin_review_withdrawal_v2', { p_withdrawal_id: id, p_decision: decision, p_note: null });
      toast(decision === 'approve' ? '已通过' : '已拒绝');
      await loadAdminWithdrawals();
      if (state.walletAdminUser) await selectAdminWalletUser(state.walletAdminUser);
    } catch (error) { toast(errorText(error)); }
  }

  function deleteAdminAccount() {
    var user = state.walletAdminUser;
    if (!user) return toast('请选择账户');
    if (state.session && user.user_id === state.session.user.id) return toast('不能注销自己');
    confirmAction('立即注销', '账户、余额和内容将永久删除。', async function () {
      await rpc('admin_delete_account_v2', { p_user_id: user.user_id, p_reason: '管理员立即注销' });
      state.walletAdminUser = null;
      resetAdminWalletUser();
      toast('账户已注销');
      await loadAdminWalletUsers(byId('admin-wallet-query').value.trim());
    });
  }

  function openAdminWalletForUser(row) {
    state.walletAdminUser = row;
    byId('admin-wallet-query').value = row.username || row.email || row.user_id;
    switchPage('wallet-admin');
  }

  function selectThemePreset(preset) {
    var allowed = ['custom', 'spring', 'festival', 'summer', 'autumn', 'school', 'winter'];
    var value = allowed.includes(preset) ? preset : 'custom';
    byId('site-theme-preset').value = value;
    document.querySelectorAll('[data-theme-preset]').forEach(function (button) {
      var active = button.dataset.themePreset === value;
      button.classList.toggle('active', active);
      button.setAttribute('aria-checked', String(active));
    });
    byId('site-background-field').classList.toggle('hidden', value !== 'custom');
  }
  ...
  async function loadSiteForm() {
    ...
    selectThemePreset(settings.theme_preset || 'custom');
    var pageMaintenance = settings.page_maintenance || {};
    if (typeof pageMaintenance === 'string') {
      try { pageMaintenance = JSON.parse(pageMaintenance); } catch (ignore) { pageMaintenance = {}; }
    }
    document.querySelectorAll('[data-maintenance-page]').forEach(function (input) {
      input.checked = !!pageMaintenance[input.dataset.maintenancePage];
    });
  }
  ...
  async function saveSite(event) {
    ...
    try {
      var pageMaintenance = {};
      document.querySelectorAll('[data-maintenance-page]').forEach(function (input) {
        pageMaintenance[input.dataset.maintenancePage] = input.checked;
      });
      var settings = first(await rpc('admin_update_site_settings_v2', {
        ...
        p_theme_preset: byId('site-theme-preset').value,
        p_page_maintenance: pageMaintenance
      }));
      applySettings(settings);
      toast('已保存');
    } catch (error) { toast(errorText(error)); }
  ...
  async function switchPage(name) {
    ...
    var adminPage = ['users', 'wallet-admin', 'settings', 'logs'].includes(name);
    if ((staffPage && !['admin', 'auditor'].includes(profile.role)) || (adminPage && profile.role !== 'admin')) name = 'home';
    state.activePage = name;
    var maintenance = state.settings && state.settings.page_maintenance;
    var blocked = profile.role !== 'admin' && maintenance && maintenance[name];
    var visiblePage = blocked ? 'page-maintenance' : name;
    if (blocked) {
      var labels = {
        home: '动态维护中', messages: '消息维护中', profile: '个人页维护中', wallet: '钱包维护中', shop: '商城维护中',
        review: '审核维护中', reports: '举报维护中', users: '用户页维护中', 'wallet-admin': '资金页维护中', settings: '设置维护中', logs: '日志维护中'
      };
      byId('page-maintenance-name').textContent = labels[name] || '此页维护中';
    }
    document.querySelectorAll('[data-page]').forEach(function (page) { page.classList.toggle('active', page.dataset.page === visiblePage); });
    ...
    try {
      ...
      else if (name === 'profile') await Promise.all([loadProfile(), loadDeletionRequest()]);
      else if (name === 'wallet') await loadWallet();
      else if (name === 'shop') return;
      else if (name === 'review') await loadReview();
      else if (name === 'reports') await loadReports();
      else if (name === 'users') await loadUsers();
      else if (name === 'wallet-admin') await loadAdminWalletPage(byId('admin-wallet-query').value.trim());
  ...
  function bindEvents() {
    ...
    byId('account-delete-request').addEventListener('click', requestAccountDeletion);
    byId('account-delete-cancel').addEventListener('click', cancelAccountDeletion);
    byId('account-delete-confirm').addEventListener('click', finalizeAccountDeletion);
    byId('wallet-refresh').addEventListener('click', loadWallet);
    byId('wallet-bills-refresh').addEventListener('click', loadWallet);
    byId('wallet-transfer-open').addEventListener('click', openTransferDialog);
    byId('wallet-recharge-open').addEventListener('click', openRechargeDialog);
    byId('wallet-withdraw-open').addEventListener('click', openWithdrawDialog);
    byId('transfer-form').addEventListener('submit', submitTransfer);
    byId('recharge-form').addEventListener('submit', submitRecharge);
    byId('withdraw-form').addEventListener('submit', submitWithdrawal);
    document.querySelectorAll('[data-recharge-amount]').forEach(function (button) {
      button.addEventListener('click', function () {
        byId('recharge-amount').value = button.dataset.rechargeAmount;
        document.querySelectorAll('[data-recharge-amount]').forEach(function (item) { item.classList.toggle('active', item === button); });
      });
    });
    byId('recharge-amount').addEventListener('input', function () {
      var value = byId('recharge-amount').value;
      document.querySelectorAll('[data-recharge-amount]').forEach(function (button) {
        button.classList.toggle('active', button.dataset.rechargeAmount === value);
      });
    });
    byId('admin-wallet-search-form').addEventListener('submit', submitAdminWalletSearch);
    byId('admin-wallet-refresh').addEventListener('click', function () {
      loadAdminWalletPage(byId('admin-wallet-query').value.trim()).catch(function (error) { toast(errorText(error)); });
    });
    byId('admin-account-bills-refresh').addEventListener('click', function () { if (state.walletAdminUser) selectAdminWalletUser(state.walletAdminUser); });
    byId('admin-withdrawals-refresh').addEventListener('click', loadAdminWithdrawals);
    byId('admin-balance-form').addEventListener('submit', adjustAdminBalance);
    byId('admin-account-delete').addEventListener('click', deleteAdminAccount);
    document.querySelectorAll('[data-theme-preset]').forEach(function (button) {
      button.addEventListener('click', function () { selectThemePreset(button.dataset.themePreset); });
    });
    byId('site-form').addEventListener('submit', saveSite);
    document.querySelectorAll('[data-refresh]').forEach(function (button) {
      button.addEventListener('click', function () {
        var handler = ({ review: loadReview, reports: loadReports, users: loadUsers, logs: loadLogs })[button.dataset.refresh];
        if (handler) handler();
      });
    });
  }
